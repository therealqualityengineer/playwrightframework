...
export class OrderPage extends BasePage
{
    ...
    async createNewOrder(jobDate : string, shift : string, certs : string, speciality : string)
    {
        ...
        await expect(orderPage.locator(this.clientNameTextbox)).toBeVisible({ timeout: 15000 });
        const orderPopupPage = new OrderPage(orderPage);
        console.log("Client name used for order creation: " + sharedData.clientName);
        ...
        await orderPopupPage.TypeText(orderPopupPage.jobDateTextbox, jobDate, 'locator');
        ...
        await orderPopupPage.Click(orderPopupPage.certificationSelect(certs), 'locator');
        await orderPopupPage.TypeText(orderPopupPage.specialitytextbox, speciality, 'locator');
        await orderPopupPage.Click(orderPopupPage.specialitySelect(speciality), 'locator');
        ...
        const orderNumber = await this.page.locator(this.orderNumberLocator).nth(0).innerText();
        sharedData.orderId = orderNumber;
        console.log("Created order number: " + orderNumber);
    }

    private async selectLookupValue(inputLocator: string, searchText: string, resultLocator: string, searchButtonIndex: number)
    {
        const input = this.page.locator(inputLocator);
        const result = this.page.locator(resultLocator).first();
        const searchButton = this.page.getByRole('button', { name: 'Search' }).nth(searchButtonIndex);
