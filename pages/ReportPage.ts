...
export class ReportPage extends BasePage {
  ...
  private tempProfilesByCertReportUrl = "reports/rw_tempprofiles.cfm?sort=cert";
  ...
  async downloadTempProfilesByCertificationReport(tempName?: string, certification = "RN") {
    await this.page.goto(this.tempProfilesByCertReportUrl);
    if (tempName) {
        await this.Click("#tfobj_textItem0", "locator", { delay: 2000 });
        await this.TypeText("#searchfor", tempName, "locator", { delay: 2000 });
        await this.page.keyboard.press("Space", { delay: 1000 });
    ...
    await this.page.locator('#csf_cert_list li').click();
    await this.page.locator(`li[title="${certification}"]`).click();
    await this.page.locator('div.CloseBtn').last().click();
    await this.page.getByRole('cell', { name: 'Excel Format (.xls)' }).getByRole('checkbox').check();
    const downloadPromise = this.page.waitForEvent('download');
    await this.page.getByRole('button', { name: 'Run Report' }).last().click();
    const downloadPage = await downloadPromise;
