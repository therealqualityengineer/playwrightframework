...
export class ClientPage extends BasePage
{
    ...
    async createNewClient(clientData : ClientData)
    {
        ...
        await this.TypeText(this.clientNameTextbox, clientData.clientname, 'locator');
        sharedData.clientName = clientData.clientname ?? '';
        console.log("Client name used for client creation: " + sharedData.clientName);
        await this.TypeText(this.addressTextbox, clientData.address ?? "16801 Addison Road", 'locator');
        await this.TypeText(this.cityTextbox, clientData.city ?? 'Addison', 'locator');
