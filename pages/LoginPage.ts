...
export class LoginPage extends BasePage {
  ...
  async verifySuccessfulLogin() {
    await expect(this.page).toHaveURL(
      "https://ctmsqa.contingenttalentmanagement.com/wfportal/index2.cfm?action=loginout",
      { timeout: 10000 },
    );
  }
  ...
  async verifyQuickLinkText(expectedQuickLinkText: any) {
    await expect(this.page.getByText(expectedQuickLinkText).nth(0)).toBeVisible(
      { timeout: 10000 },
    );
  }
}
