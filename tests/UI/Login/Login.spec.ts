...
test("@smoke Verify user able to login successfully parallel", async ({
  loginPage,
}) => {
...
test("@smoke Verify Quick link text in main menu page", async ({
  loginPage,
}) => {
  await loginPage.login(users.validUser1.username, users.validUser1.password);
  await loginPage.verifySuccessfulLogin();
...
test("@smoke Verify Quick link text in admin user page", async ({
  loginPage,
}) => {
  await loginPage.login(users.validUser1.username, users.validUser1.password);
  await loginPage.verifySuccessfulLogin();
